import React, { useEffect, useRef, useContext } from "react"; //use effect is like all lifecycle hooks mashed
...
const cockpit = (props) => {
  ...
  //toggleBtnRef.current.click(); wont work here cuz it only works after rendered. so we use it in useEffect
  ...
  //using context. better form. class style and old form are in Person.js
  ...
  return (
    <div>
      ...
      <button
        key="login"
        onClick={
          authContext.login /**better way to use context. others in person.js */
        }
      >
        Login
      </button>
